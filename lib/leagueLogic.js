import { mergeTeamWithLiveData, getTotalStartingPoints } from "./fplData";
import { applyAutoSubstitutions } from "./lineup";
...
export function computeStandings(matchups) {
  ...
  for (const matchup of matchups.filter(isFinished)) {
    ...
    if (matchup.manager_1_score === matchup.manager_2_score) {
      r1.draws += 1;
      r2.draws += 1;
    } else if (matchup.manager_1_score > matchup.manager_2_score) {
      r1.wins += 1;
      r2.losses += 1;
    } else {
      r2.wins += 1;
  ...
  return [...recordByName.values()].sort(
    (a, b) =>
      3 * b.wins + b.draws - (3 * a.wins + a.draws) ||
...
export function computeMatchupScores(
  ...
  for (const name of names) {
    ...
    const rows = rosterRowsByManagerId.get(manager.id) ?? [];
    let players = mergeTeamWithLiveData(rows, bootstrap, live);
    if (autoSub) {
      players = applyAutoSubstitutions(players).players;
    }
