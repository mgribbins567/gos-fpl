...
export function buildPlayerGameweekHistory({
  ...
  const rows = history
    ...
    .map((data) => {
      ...
      const row = {
        GW: data.round,
        VS: `${data.team_h_score}-${data.team_a_score}`,
        OPP: ` ${data.was_home ? "" : "@"}${opponent.short_name}`,
        ...
        XG: Math.round(data.expected_goals * 100) / 100 || 0,
        XA: Math.round(data.expected_assists * 100) / 100 || 0,
        XGI: Math.round(data.expected_goal_involvements * 100) / 100 || 0,
      };
  ...
  const totals = rows.reduce((acc, row) => {
    for (const col of NUMERIC_HISTORY_COLUMNS) {
      acc[col] = Math.round(((acc[col] ?? 0) + (row[col] ?? 0)) * 100) / 100;
    }
    return acc;
  }, {});
  totals.GW = "Total";
