...
export async function getLeagues(supabase) {
...
export async function getCupMatchupsByCup(supabase, cupName) {
  ...
  return data.map((matchup) => {
    const { Gameweek, ...rest } = matchup;

    return {
      ...rest,
      gameweekNumber: Gameweek?.gameweek || null,
    };
  });
}
...
export async function getManagerLeagueIds(supabase, seasonId) {
  const { data, error } = await supabase
    .from("LeagueManager")
    .select("league_id, manager_id")
    .eq("season_id", seasonId);
  if (error) throw new Error(error.message);
  return new Map(data.map((row) => [row.manager_id, row.league_id]));
}
